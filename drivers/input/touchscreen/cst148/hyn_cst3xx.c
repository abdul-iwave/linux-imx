...
#define    HYN_RESET_PIN  1   /* IWG37M: Reset: Defining touch reset gpio number for GPIO1_1 */ 
...
static unsigned int  g_irq_pin = 8; /* IWG37M: Interrupt: Defining interrupt gpio number for GPIO1_8 */
...
static void cst3xx_reset_ic(unsigned int ms)
{
	...
	/* IWG37M: Touch Software Reset not used */
	unsigned char buf[4];	
	buf[0] = 0xD1;	
	buf[1] = 0x0E;	 	
...
#ifdef CONFIG_IWG37M
	/* IWG37M: Set Touch IRQ GPIO as Input */
	gpio_direction_input(ts->pdata->irq_gpio);
#endif
